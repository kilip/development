...
class List extends Component {
    componentDidMount() {
        this.props.list(
            ...
            && '/{{{ name }}}?page=' + this.props.match.params.page
        );
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (this.props.match.params.page !== nextProps.match.params.page) {
            nextProps.list(
                ...
                && '/{{{ name }}}?page=' + nextProps.match.params.page
            );
        }
    }
    ...
    render() {
        return (
            <div>
              ...
              <p><Link to="/{{{ name }}}/create" className="btn btn-primary">Create</Link></p>
              ...
              <table className="table table-responsive table-striped table-hover">
                ...
                {this.props.data['{{{ hydraPrefix }}}member'] && this.props.data['{{{ hydraPrefix }}}member'].map(item =>
                    <tr key={item['@id']}>
                        ...
                        {{/each}}
                        <td>
                            <Link to={`/{{{ name }}}/show/${encodeURIComponent(item['id'])}`}>
                                <span className="fa fa-search" aria-hidden="true"/>
                                <span className="sr-only">Show</span>
                            </Link>
                        </td>
                        <td>
                            <Link to={`/{{{ name }}}/edit/${item['id']}`}>
    ...
    pagination() {
        ...
        _.each(view,function(value,key,view){
            value = value.replace('/api/{{{ name }}}?page=','/{{{ name }}}/');
            view[key] = value
        });
